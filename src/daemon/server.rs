...
async fn chat(
    ...
    tokio::task::spawn(async move {
        if let Err(e) = run_agent_turn(state_clone, &session_id, req, tx.clone()).await {
...
async fn run_agent_turn(
    ...
    // Create a Console that forwards SinkLine events via std mpsc.
    let (sink_tx_std, sink_rx_std) = std::sync::mpsc::channel::<SinkLine>();
    let (approval_tx_std, _approval_rx_std) = std::sync::mpsc::channel::<ApprovalRequest>();
    let console = Console::new(sink_tx_std, approval_tx_std);

    // Bridge std mpsc to tokio mpsc for SSE streaming.
        ...
        while let Some(sl) = sink_rx.recv().await {
            let event = match sl {
                ...
                SinkLine::ToolInput(s) => {
                    // Parse tool name from "name args" format.
...
async fn approve(
    ...
    Json(_req): Json<ApprovalResponse>,
) -> Result<Json<serde_json::Value>, StatusCode> {
    // TODO: Forward approval decision to the active agent turn.
    Ok(Json(json!({ "status": "ok" })))
}

async fn cancel(Path(_session_id): Path<String>) -> Result<Json<serde_json::Value>, StatusCode> {
    // TODO: Cancel the active turn for this session.
    Ok(Json(json!({ "status": "ok" })))
}
