...
///
/// The preferred location is `dex/config.json`; if it does not exist we fall
...
pub(crate) fn path() -> Option<PathBuf> {
    ...
    let (primary, legacy) = match env::var_os("XDG_CONFIG_HOME") {
        Some(dir) => (
            PathBuf::from(&dir).join("dex/config.json"),
            PathBuf::from(&dir).join("ak/config.json"),
        ),
        None => {
            let home = env::var_os("HOME")?;
            (
                PathBuf::from(&home).join(".config/dex/config.json"),
