...
pub(crate) fn system_prompt(skills: &[Skill]) -> String {
    ...
    prompt.push_str("\n\nStitching: intermediate tool output does not need to reach the conversation. When you need a distilled result (matched files, counts, short excerpts, an aggregate) rather than full outputs, run the whole pipeline in ONE bash call: the oye binary is available as \"$OYE_BIN\" and `\"$OYE_BIN\" run <tool> <key>=<value>...` executes read/grep/find/git locally with raw text on stdout (exit 1 on error). Local calls are free — only what the script prints enters the conversation. Example: `\"$OYE_BIN\" run grep pattern=TODO output_mode=files | while IFS= read -r f; do \"$OYE_BIN\" run read \"path=$f\" limit=3; done`. Stitching is read-only; use the dedicated tools whenever you need to see the full output yourself.");
